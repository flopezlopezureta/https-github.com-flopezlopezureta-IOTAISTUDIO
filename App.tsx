...
const App: React.FC = () => {
  ...
  const refreshData = useCallback(() => {
    if (currentUser) {
      const filteredDevices = databaseService.getDevices(currentUser);
      const allCompanies = databaseService.getCompanies();
  ...
  useEffect(() => {
    ...
    // --- SIMULADOR DE INGESTA DE DATOS (MECANISMO DE PRUEBA) ---
    // Esto permite que el usuario pruebe la "recepción" de datos llamando a esta función desde la consola.
    (window as any).simulateDeviceUpload = (mac: string, value: number) => {
      const device = databaseService.getDevices().find(d => d.mac_address === mac);
      if (device) {
        databaseService.updateDevice(device.id, { value, status: 'online' });
        refreshData();
        console.log(`%c[Selcom IoT] Datos recibidos para ${mac}: ${value}`, "color: #22d3ee; font-weight: bold;");
      } else {
        console.error(`Dispositivo con MAC ${mac} no encontrado.`);
      }
    };
    ...
    const ticker = setInterval(() => {
      ...
      devices.forEach((d) => {
        ...
        if (now - lastUpdate >= samplingIntervalMs) {
          setLastUpdates(prev => ({ ...prev, [d.id]: now }));
          const newValue = Math.max(0, d.value + (Math.random() - 0.5) * 0.8);
          databaseService.updateDevice(d.id, { value: newValue });
          setDevices(prev => prev.map(dev => dev.id === d.id ? { ...dev, value: newValue } : dev));
        }
      });
    }, 3000);
  ...
  const renderContent = () => {
    ...
    switch(viewMode) {
      case 'dashboard': 
        return <Dashboard user={currentUser} devices={devices} onSelectDevice={(d) => { setSelectedDevice(d); setViewMode('device-detail'); }} />;
      case 'custom-dashboard':
        return <GrafanaDashboard devices={devices} />;
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Gestión de Dispositivos Conectados</p>
              </div>
            ...
            {devices.length === 0 ? (
              <div className="bg-[#1e293b] p-20 rounded-[2rem] border border-slate-800/50 text-center">
                <p className="text-slate-500 text-xs font-black uppercase tracking-[0.3em]">No hay dispositivos registrados</p>
              </div>
            ) : (
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex gap-4">
                        <div className="p-3 bg-slate-900 rounded-2xl text-cyan-400 border border-slate-800">
                          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2-2v10a2 2 0 002 2z"></path></svg>
                        </div>
                        <div>
  ...
  if (!currentUser) return <Login onLogin={(u) => { setCurrentUser(u); localStorage.setItem('selcom_auth_user', JSON.stringify(u)); }} />;
        ...
        <div className="container mx-auto max-w-7xl">
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-white text-2xl font-black uppercase tracking-tight">Selcom Hub</h2>
            ...
            <div className="hidden md:flex px-4 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-lg items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span>
              <span className="text-[9px] font-black uppercase text-amber-500 tracking-widest">Entorno de Pruebas Local</span>
            </div>
          </div>
          {renderContent()}
        </div>
