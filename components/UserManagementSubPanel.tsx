...
const UserManagementSubPanel: React.FC<UserManagementSubPanelProps> = ({ company, onBack }) => {
  ...
  // Fix: Carga asíncrona de usuarios para evitar problemas con Promises
  const refreshUsers = async () => {
    const companyUsers = await databaseService.getUsers(company.id);
    setUsers(companyUsers);
  };

  // Fix: Implementar manejo asíncrono en el guardado de usuarios
  ...
  // Fix: Esperar la eliminación asíncrona
  ...
  // Fix: Esperar la actualización de contraseña en el servidor
  const handleResetPassword = async (user: User) => {
    const newPass = Math.random().toString(36).slice(-8).toUpperCase();
    await databaseService.updateUser(user.id, { password: newPass });
